...
public class Kart2 : MonoBehaviour
{
    ...
    public Kart2Light Light;
    public Rigidbody Rigidbody;
    public int kartNumber = 2;
        ...
        if (KartSettings.maxPlayers > 1)
        {
            ...
            if (start)
            {
                ...
                if (win == false && go)
                {
                    if (ITEM && itemUp)
                    {
                        ...
                        if (item == 1 && flipped == false && drift == false)
                        {
                            ...
                            KartSettings.Audio.StartSound("Booster 2", 0);
                        }
                ...
                if (KartSettings.maxPlayers >= 2)
                {
                    ...
                    {
                        KartSettings.Audio.StartSound("Idle 2", 0);
                        idleSound = true;
                    }
                    else if (accelerateSound)
                    {
                        KartSettings.Audio.StopSound("Idle 2");
                        idleSound = false;
                    }

                    if (ACCELERATE && accelerateSound == false && go && win == false && flipped == false && hit == false && drift == false && KartSettings.Audio.Audio[67].isPlaying == false && offRoad == false)
                    {
                        KartSettings.Audio.StartSound("Drive 2", 0);
                        accelerateSound = true;
                    }
                    else if ((ACCELERATE == false || go == false || win || flipped || hit || drift || KartSettings.Audio.Audio[67].isPlaying) && accelerateSound)
                    {
                        KartSettings.Audio.Play("Decelerate");
                        KartSettings.Audio.StopSound("Drive 2");
                    ...
                    {
                        KartSettings.Audio.StartSound("Brake 2", 0);
                        brakeSound = true;
                    }
                    else if (speed <= 0 || REVERSE == false || accelerateSound || go == false || win || flipped || hit || drift)
                    {
                        KartSettings.Audio.StopSound("Brake 2");
                        brakeSound = false;
                    }

                    if (speed <= 0 && REVERSE && reverseSound == false && accelerateSound == false && go && win == false && flipped == false && hit == false && drift == false)
                    {
                        KartSettings.Audio.StartSound("Reverse 2", 0);
                        reverseSound = true;
                    }
                    else if (speed > 0 || REVERSE == false || accelerateSound || go == false || win || flipped || hit || drift)
                    {
                        KartSettings.Audio.StopSound("Reverse 2");
                        reverseSound = false;
                    }

                    if (drift && driftSound == false)
                    {
                        KartSettings.Audio.StartSound("Drift 2", 0);
                        driftSound = true;
                    }
                    else if (drift == false)
                    {
                        KartSettings.Audio.StopSound("Drift 2");
                        driftSound = false;
                    }

                    if (offRoad && speed > 0 && offRoadSound == false && boost == false)
                    {
                        KartSettings.Audio.StartSound("Offroad 2", 0);
                        offRoadSound = true;
                    }
                    else if (offRoad == false || speed <= 0 || boost)
                    {
                        KartSettings.Audio.StopSound("Offroad 2");
                        offRoadSound = false;
                    }

                    if (onIce && speed > 0 && iceSound == false)
                    {
                        KartSettings.Audio.StartSound("Ice 2", 0);
                        iceSound = true;
                    }
                    else if (onIce == false || speed <= 0)
                    {
                        KartSettings.Audio.StopSound("Ice 2");
                        iceSound = false;
                    }

                    if (speed < maxSpeed)
                    {
                        KartSettings.Audio.StopSound("Booster 2");
                    }
                }
    ...
    {
        // Touching Offroad
        if (other.tag == "Offroad" && KartSettings.character[2] != "Kelley")
        {
            offRoad = true;
        }

        // Touching Ice
        if (other.tag == "Ice" && KartSettings.character[2] != "Justin")
        {
            onIce = true;
        }
