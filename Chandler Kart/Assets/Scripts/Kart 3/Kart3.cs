...
public class Kart3 : MonoBehaviour
{
    public PlayerInput Input;

    public KartSettings KartSettings;
    public Camera3 Camera;
    public Kart3Light Light;
    public Rigidbody Rigidbody;
    public int kartNumber = 3;
    ...
    void Start()
    {
        ...
        transform.position = new Vector3(0, -100, -15);
        ...
        if (KartSettings.maxPlayers > 2)
        {
            ...
            if (start)
            {
                ...
                if (win == false && go)
                {
                    if (ITEM && itemUp)
                    {
                        ...
                        if (item == 1 && flipped == false && drift == false)
                        {
                            ...
                            KartSettings.Audio.StartSound("Booster 3", 0);
                        }
                ...
                if (KartSettings.maxPlayers >= 3)
                {
                    ...
                    {
                        KartSettings.Audio.StartSound("Idle 1", 0);
                        idleSound = true;
                    }
                    else if (accelerateSound)
                    {
                        KartSettings.Audio.StopSound("Idle 1");
                        idleSound = false;
                    }

                    if (ACCELERATE && accelerateSound == false && go && win == false && flipped == false && hit == false && drift == false && KartSettings.Audio.Audio[68].isPlaying == false && offRoad == false)
                    {
                        KartSettings.Audio.StartSound("Drive 1", 0);
                        accelerateSound = true;
                    }
                    else if ((ACCELERATE == false || go == false || win || flipped || hit || drift || KartSettings.Audio.Audio[68].isPlaying) && accelerateSound)
                    {
                        KartSettings.Audio.Play("Decelerate");
                        KartSettings.Audio.StopSound("Drive 1");
                    ...
                    {
                        KartSettings.Audio.StartSound("Brake 1", 0);
                        brakeSound = true;
                    }
                    else if (speed <= 0 || REVERSE == false || accelerateSound || go == false || win || flipped || hit || drift)
                    {
                        KartSettings.Audio.StopSound("Brake 1");
                        brakeSound = false;
                    }

                    if (speed <= 0 && REVERSE && reverseSound == false && accelerateSound == false && go && win == false && flipped == false && hit == false && drift == false)
                    {
                        KartSettings.Audio.StartSound("Reverse 1", 0);
                        reverseSound = true;
                    }
                    else if (speed > 0 || REVERSE == false || accelerateSound || go == false || win || flipped || hit || drift)
                    {
                        KartSettings.Audio.StopSound("Reverse 1");
                        reverseSound = false;
                    }

                    if (drift && driftSound == false)
                    {
                        KartSettings.Audio.StartSound("Drift 1", 0);
                        driftSound = true;
                    }
                    else if (drift == false)
                    {
                        KartSettings.Audio.StopSound("Drift 1");
                        driftSound = false;
                    }

                    if (offRoad && speed > 0 && offRoadSound == false && boost == false)
                    {
                        KartSettings.Audio.StartSound("Offroad 1", 0);
                        offRoadSound = true;
                    }
                    else if (offRoad == false || speed <= 0 || boost)
                    {
                        KartSettings.Audio.StopSound("Offroad 1");
                        offRoadSound = false;
                    }

                    if (onIce && speed > 0 && iceSound == false)
                    {
                        KartSettings.Audio.StartSound("Ice 1", 0);
                        iceSound = true;
                    }
                    else if (onIce == false || speed <= 0)
                    {
                        KartSettings.Audio.StopSound("Ice 1");
                        iceSound = false;
                    }

                    if (speed < maxSpeed)
                    {
                        KartSettings.Audio.StopSound("Booster 3");
                    }
                }
    ...
    {
        // Touching Offroad
        if (other.tag == "Offroad" && KartSettings.character[3] != "Kelley")
        {
            offRoad = true;
        }

        // Touching Ice
        if (other.tag == "Ice" && KartSettings.character[3] != "Justin")
        {
            onIce = true;
        }
