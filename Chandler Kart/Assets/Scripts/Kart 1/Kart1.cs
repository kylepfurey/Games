...
public class Kart1 : MonoBehaviour
{
    ...
    public Kart1Light Light;
    public Rigidbody Rigidbody;
    public int kartNumber = 1;
    ...
    void Start()
    {
        ...
        transform.position = new Vector3(0, -100, -5);
        ...
        if (KartSettings.maxPlayers > 0)
        {
            ...
            if (start)
            {
                ...
                if (win == false && go)
                {
                    if (ITEM && itemUp)
                    {
                        ...
                        if (item == 1 && flipped == false && drift == false)
                        {
                            ...
                            KartSettings.Audio.Play("Booster 1");
                        }
                ...
                if (KartSettings.maxPlayers >= 1)
                {
                    ...
                    if (ACCELERATE && accelerateSound == false && go && win == false && flipped == false && hit == false && drift == false && KartSettings.Audio.Audio[66].isPlaying == false && offRoad == false)
                    {
                        KartSettings.Audio.StartSound("Drive 1", 0);
                        accelerateSound = true;
                    }
                    else if ((ACCELERATE == false || go == false || win || flipped || hit || drift || KartSettings.Audio.Audio[66].isPlaying) && accelerateSound)
                    {
    ...
    {
        // Touching Offroad
        if (other.tag == "Offroad" && KartSettings.character[1] != "Kelley")
        {
            offRoad = true;
        }

        // Touching Ice
        if (other.tag == "Ice" && KartSettings.character[1] != "Justin")
        {
            onIce = true;
        }
