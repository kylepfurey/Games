if (global.turn = 0 and global.enemyhp > 0 and global.hp > 0 and global.cooldown <= 0) {
	
	audio_play_sound(snd_spc, 1, false);

	with (obj_animation){
		sprite_index = spr_bash
		x = 256
		y = 160
		alarm[2] = 40
	}

	global.atk = global.atk + 1;
	global.def = global.def - 1;
	if (global.enemydef = 0){
		def = 1}
	if (global.enemydef > 0){
		global.enemydef = global.enemydef - 1
		}
