...
public static class VectorThree
{
    ...
    // Returns whether the new point is convex relative to a given mesh
    public static bool IsConvex(Mesh mesh)
    {
        // Loop through triangles
        for (int i = 0; i < mesh.triangles.Length; i += 3)
        {
            // Make triangle
            Plane triangle = new Plane(mesh.vertices[mesh.triangles[i]], mesh.vertices[mesh.triangles[i + 1]], mesh.vertices[mesh.triangles[i + 2]]);
            ...
            for (int j = 0; j < mesh.vertices.Length; j++)
            {
                ...
                // Check that the point is not outside the triangle
                if (triangle.GetSide(mesh.vertices[j]))
                {
                    // A point is concave
                    return false;
                }
    ...
    public static bool IsConcave(Vector3[] vertices, int[] triangles)
    {
        // Loop through triangles
        for (int i = 0; i < triangles.Length; i += 3)
        {
    ...
    // Returns whether the new point is concave relative to a given mesh
    public static bool IsConcave(Mesh mesh)
    {
        // Loop through triangles
        for (int i = 0; i < mesh.triangles.Length; i += 3)
        {
    ...
    // Returns whether the new point is on the same side as a given mesh
    public static bool IsFlat(Mesh mesh)
    {
        // Loop through triangles
        for (int i = 0; i < mesh.triangles.Length; i += 3)
        {
            ...
            for (int j = 0; j < mesh.vertices.Length; j++)
            {
                ...
                // Check that the point is not outside the triangle
                if (triangle.GetDistanceToPoint(mesh.vertices[j]) < 0)
                {
                    // A point is not the same side
                    return false;
                }
