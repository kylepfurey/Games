...
public class Graph : MonoBehaviour
{
    ...
    // Testing routes with mouse click
    private void Update()
    {
        if (Input.GetMouseButtonDown(0))
        {
            if (testRouteWithMouse && startNode != null)
            {
                // Get camera click
                ...
                if (Physics.Raycast(mouseRaycast, out hit, 100))
                {
                    if (hit.collider.GetComponent<Node>())
                    {
                        // Set our end node to there
                        Node node = hit.collider.GetComponent<Node>();
                        endNode = node;

                        // Pathfind to our end node
                        List<Node> route = Pathfinding.Pathfind(algorithm, startNode, endNode, heuristicScale);

                        // Draw lines to represent the route (if applicable)
                        if (route.Count > 0)
                        {
                            Debug.DrawLine(startNode.transform.position, route[0].transform.position, new Vector4(Mathf.Abs(renderConnectionColor.r - 1), Mathf.Abs(renderConnectionColor.g - 1), Mathf.Abs(renderConnectionColor.b - 1), renderConnectionColor.a), 2);

                            for (int i = 0; i < route.Count - 1; i++)
                            {
                                Debug.DrawLine(route[i].transform.position, route[i + 1].transform.position, new Vector4(Mathf.Abs(renderConnectionColor.r - 1), Mathf.Abs(renderConnectionColor.g - 1), Mathf.Abs(renderConnectionColor.b - 1), renderConnectionColor.a), 2);
                            }
                        }
                        ...
                        endNode.transform.position = position;
