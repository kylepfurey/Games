...
using UnityEngine;
//DIJKSTRA SEARCH
//Again, this script is similar to the BreadthFirstSearch, except it uses the principles of Dijkstra's algorithm
//Although similarly performance-intensive as BFS (if not a bit more), this script will reliably return the shortest path between two nodes
//It works by figuring out the lowest possible cost to reach each searched node from the starting point, and then chooses a path by checking the cost values as it traces the path back.
//WRITTEN BY LIAM SHELTON
public class DijkstraSearch : SearchAlgorithm_Template
{
    ...
    {
        PathGraph.Log($"DIJKSTRA Searching node {currentNode.gameObject.name}");
        ...
        //The "breadth first" search behavior is implemented here, same as it was for BreadthFirstSearch. It looks like only 1 line, but changing this line will change how the search pattern behaves.
        //This algorithm will seach nodes in a circle around the starting point. This is resource-intensive!
