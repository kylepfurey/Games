...
namespace UnityMovementAI
{
    ...
    public class WallAvoidance : MonoBehaviour
    {
        ...
        public Vector3 GetSteering(Vector3 facingDir)
        {
            ...
            // If the target can fly, try flying upwards to avoid short walls.
            if (rb.canFly)
            {
                targetPostition += Vector3.up;
            }
        ...
        bool GenericCast(Vector3 direction, out GenericCastHit hit, float distance = Mathf.Infinity)
        {
            ...
            if (rb.is3D)
            {
                ...
                {
                    result = Physics.Raycast(origin, direction, out h, distance, castMask.value, QueryTriggerInteraction.Ignore);
                }
                else
                {
                    result = Physics.SphereCast(origin, (rb.Radius * 0.5f), direction, out h, distance, castMask.value, QueryTriggerInteraction.Ignore);
                }
