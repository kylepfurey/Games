...
namespace Octree
{
    ...
    public partial class PointOctree<T>
    {
        ...
        private class Node
        {
            ...
            /// <summary>
            /// Return objects that are not equal to ignoredElement and are within <paramref name="maxDistance"/> of the specified position.
            /// </summary>
            /// <param name="position">The position.</param>
            /// <param name="maxDistance">Maximum distance from the position to consider.</param>
            /// <param name="result">List result.</param>
            ...
            /// <returns>Objects within range.</returns>
            public void GetNearby(ref Vector3 position, float maxDistance, List<T> result, T ignoredElement)
            {
                // Does the node contain this position at all?
                // Note: Expanding the bounds is not exactly the same as a real distance check, but it's fast.
                // TODO: Does someone have a fast AND accurate formula to do this check?
                ...
                for (int i = 0; i < _objects.Count; i++)
                {
                    if (Vector3.Distance(position, _objects[i].Pos) <= maxDistance && !_objects[i].Obj.Equals(ignoredElement))
                    {
                        result.Add(_objects[i].Obj);
                    }
                }
                ...
                if (_children != null)
                {
                    for (int i = 0; i < 8; i++)
                    {
                        _children[i].GetNearby(ref position, maxDistance, result, ignoredElement);
                    }
                }
            }
