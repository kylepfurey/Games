...
namespace Octree
{
    ...
    public partial class PointOctree<T>
    {
        ...
        /// <param name="position">The position. Passing as ref to improve performance since it won't have to be copied.</param>
        /// <param name="maxDistance">Maximum distance from the position to consider.</param>
        /// <param name="ignoredObject">The object that is not included in the return value.</param>
        /// <returns>Objects within range.</returns>
        public T[]GetNearby(Vector3 position, float maxDistance, T ignoredObject)
        {
            List<T> collidingWith = new List<T>();
            _rootNode.GetNearby(ref position, maxDistance, collidingWith);
            collidingWith.Remove(ignoredObject);
            return collidingWith.ToArray();
        }
        ...
        /// <summary>
        /// Returns objects that are not equal to ignoredElement and are within <paramref name="maxDistance"/> of the specified position.
        /// If none, returns false. Uses supplied list for results.
        /// </summary>
        /// <param name="position">The position. Passing as ref to improve performance since it won't have to be copied.</param>
        /// <param name="maxDistance">Maximum distance from the position to consider.</param>
        /// <param name="nearby">Pre-initialized list to populate.</param>
        /// <param name="ignoredElement">The element that needs to be ignored.</param>
        /// <returns><c>true</c> if items are found, <c>false</c> otherwise.</returns>
        public bool GetNearbyNonAlloc(Vector3 position, float maxDistance, List<T> nearby, T ignoredElement)
        {
            nearby.Clear();
            _rootNode.GetNearby(ref position, maxDistance, nearby, ignoredElement);
            return nearby.Count > 0;
        }
        ...
        public void GetAllNonAlloc(ref List<T> allObjects, bool shouldClearListFirst = true) 
        {
            if (shouldClearListFirst)
                allObjects.Clear();
            _rootNode.GetAll(allObjects);
        }
